...
data_vac %>% 
  filter(FIPS > 66000) %>% count(FIPS, Recip_State)
...
data_merged <- data_vac_newest %>%
  inner_join(data_demo %>% select(-"County"), by = "FIPS") %>%
  inner_join(df_trump  %>% select(-c(state, county)), by = "FIPS") 
```
...
- How to address it?
  ...
  3. Group data, calculate correlation each month
  4. Plot?
