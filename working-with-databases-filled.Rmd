...
df_tmp <- dbGetQuery(db, "select vac.*, demo.FIPS from vac 
...
df_new2 <- db_vac %>% 
  filter(Date == "2021-08-14" & Series_Complete_Pop_Pct > 10) %>% collect()
```
#### Ordering and aggregation
...
db_vac %>% filter(Date == "2021-07-27") %>%
  ...
  summarize(sd(Administered_Dose1_Recip_12Plus)) 
